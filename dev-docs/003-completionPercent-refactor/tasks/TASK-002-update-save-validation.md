...
- [ ] Validation fails if `age` is missing or <= 0
- [ ] Validation fails if `gender` is empty or unselected
- [ ] Validation fails if `race` is empty or unselected
- [ ] Error messages display inline for each field
- [ ] Global "Please fix validation errors" appears when any required field missing
- [ ] Existing name/summary/backstory validations still work
