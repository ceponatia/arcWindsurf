...
| ----------- | ------------------------------------------- |
| **Status**  | ✅ 💬 🔀 🖥️ 💾                              |
| **Type**    | `BodyMap` (per-region scent/texture/visual) |
...
The Sensory Agent uses `body` data for smell and touch intents:

1. Player says: "I smell her hair"
2. Intent detection extracts: `{ action: 'smell', target: 'hair' }`
3. `resolveBodyRegion('hair')` returns `'hair'` region
4. `getRegionScent(character.body, 'hair')` returns scent data
5. Scent data is formatted and returned to player
