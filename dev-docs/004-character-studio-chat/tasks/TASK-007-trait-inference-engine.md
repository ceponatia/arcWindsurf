# TASK-007: Trait Inference Engine
...
Create a TraitInferenceEngine that analyzes conversation exchanges to infer character personality traits with confidence scoring and contradiction detection.
...
export class TraitInferenceEngine {
  ...
  private accumulatedEvidence: Map<string, InferredTrait[]> = new Map();
  ...
  getHighConfidenceTraits(): InferredTrait[] {
    ...
    for (const [path, traits] of this.accumulatedEvidence) {
      ...
      if (finalConfidence >= 0.6) {
        ...
        const mostRecentValue = traits[traits.length - 1].value;
...
Update the `inferTraits` actor in `studio-machine.ts`:

```typescript
inferTraits: fromPromise(async ({ input }) => {
  const ctx = input as StudioMachineContext;
  const engine = new TraitInferenceEngine({ llmProvider: ctx.llmProvider });

  // Get the last user message and character response
  const messages = ctx.conversation;
  const lastCharMsg = messages.filter(m => m.role === 'character').slice(-1)[0];
  const lastUserMsg = messages.filter(m => m.role === 'user').slice(-1)[0];

  if (!lastCharMsg || !lastUserMsg) return [];

  return engine.inferFromExchange(
    lastUserMsg.content,
    lastCharMsg.content,
    ctx.profile
  );
