...
-- Seed a starter Location Map for the test setting.
-- This matches @minimal-rpg/schemas LocationMapSchema and is compatible with the Location Builder.
...
VALUES (
  ...
  'test-setting-001',
  'system',
  'Lanse Creuse High - Starter Map',
  'A small starter location map for the seeded test setting.',
  TRUE,
  '[
    {
    ...
    },
    {
      ...
      "description": "Lockers line both sides, the fluorescent lights hum, and the scent of floor wax never quite goes away.",
  ...
  'test-loc-hallway-001',
  ARRAY['test', 'school', 'modern']
)
ON CONFLICT (id) DO NOTHING;
