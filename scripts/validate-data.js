...
function validateCharacter(obj, file) {
  ...
  for (const k of requiredScalars) {
    if (!obj[k] || typeof obj[k] !== 'string' || obj[k].length === 0) errs.push(`${k} must be non-empty string`)
  }
  ...
  if (!('personality' in obj)) {
    ...
    if (typeof pers === 'string') {
      ...
      if (pers.length === 0 || pers.some(p => typeof p !== 'string' || p.length === 0)) errs.push('personality array must contain non-empty strings')
    } else {
  ...
  if (obj.age !== undefined) {
    if (typeof obj.age !== 'number' || !Number.isInteger(obj.age) || obj.age < 16 || obj.age > 120) errs.push('age must be integer 16..120')
  }
  // Goals
  ...
  if (obj.style) {
    ...
    for (const [key, allowed] of Object.entries(enums)) {
      if (style[key] && !allowed.includes(style[key])) errs.push(`style.${key} invalid (${style[key]}) must be one of ${allowed.join('|')}`)
    }
  }
  ...
  if (obj.appearance) {
    if (typeof obj.appearance === 'string') {
      ...
      // height categorical
      if (ap.height && !['short','average','tall'].includes(ap.height)) errs.push(`appearance.height invalid (${ap.height}) must be short|average|tall`)
      if (ap.build && !['slight','average','athletic','heavy'].includes(ap.build)) errs.push(`appearance.build invalid (${ap.build}) must be slight|average|athletic|heavy`)
      if (ap.features && (!Array.isArray(ap.features) || ap.features.some(f => typeof f !== 'string' || f.length === 0))) errs.push('appearance.features must be array of non-empty strings')
      if (ap.skinTone && (typeof ap.skinTone !== 'string' || ap.skinTone.length === 0)) errs.push('appearance.skinTone must be non-empty string')
      if (ap.description && (typeof ap.description !== 'string' || ap.description.length === 0)) errs.push('appearance.description must be non-empty string')
      // Deprecated / disallowed fields
      const deprecated = ['heightCm','nubile','breastSize','clothingStyle','distinguishingFeatures']
  ...
  if (obj.scent) {
    if (typeof obj.scent !== 'object' || Array.isArray(obj.scent)) {
      ...
      const sc = obj.scent
      if (sc.hairScent && !['floral','citrus','fresh','herbal','neutral'].includes(sc.hairScent)) errs.push(`scent.hairScent invalid (${sc.hairScent})`)
      if (sc.bodyScent && !['clean','fresh','neutral','light musk'].includes(sc.bodyScent)) errs.push(`scent.bodyScent invalid (${sc.bodyScent})`)
      if (sc.perfume && (typeof sc.perfume !== 'string' || sc.perfume.length === 0 || sc.perfume.length > 40)) errs.push('scent.perfume must be 1..40 chars')
