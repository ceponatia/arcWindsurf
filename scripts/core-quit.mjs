...
async function teardown() {
  ...
  const reportLine = (service) => {
    ...
    else if (service.pidKill && !service.pidKill.skipped && !service.pidKill.error)
      parts.push('force pid kill sent');

    service.portKills.forEach((attempt) => {
      const killed = attempt.killed.length ? `killed ${attempt.killed.join(',')}` : null;
      const killErr = attempt.killErrors.length
        ? `errors ${attempt.killErrors.map((e) => `${e.pid}:${e.message}`).join(',')}`
        : null;
  ...
  if (api.busy || web.busy) {
    console.log(
      '[core:quit] Remaining busy ports detected. Investigate manually (e.g., lsof -i :PORT).',
    );
  } else {
